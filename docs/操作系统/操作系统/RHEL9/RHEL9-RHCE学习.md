...
```yaml
[devops@workstation ansible]$ ansible dev -m user -a 'name=www uid=2000 shell=/sbin/nologin'
```
...
```yaml
[devops@workstation ansible]$ ansible dev -m group -a 'name=apache gid=800'
```
...
```yaml
[devops@workstation ansible]$ ansible dev -m script -a 'creates=/tmp/a.txt test.sh'
```

```yaml
[difu@control ansible]$ ansible all -m script -a 'creates=/etc/fstab test.sh' 
```
...
```yaml
[devops@workstation ansible]$ ansible dev -m yum -a 'name=httpd state=latest'
```

<font style="color:rgb(0,0,0);">安装包组 </font>

```yaml
[devops@workstation ansible]$ ansible dev -m yum -a 'name="@Development Tools" state=latest'
```
...
```yaml
[devops@workstation ansible]$ ansible all -m service -a 'name=sshd state=restarted'
```
...
```yaml
[devops@workstation ansible]$ ansible all -m copy -a 'src=test.txt dest=/etc/motd mode=644'
```
...
<font style="color:rgb(0,0,0);">2.在指定的目录中定义 </font>
...
**<font style="color:rgb(0,0,0);">defaults/main.yml</font>**<font style="color:rgb(0,0,0);">: 用于存放roles中的默认变量，此文件中的变量</font><font style="color:rgb(223,64,42);">优先 级最低 </font>

**<font style="color:rgb(0,0,0);">files: </font>**<font style="color:rgb(0,0,0);">用于存放需要拷贝的文件 </font>

**<font style="color:rgb(0,0,0);">handlers/mail.yml: </font>**<font style="color:rgb(0,0,0);">用于存放handler任务，当roles中需要调用 handler时就会在此文件中查找相应的handler（触发）. </font>

**<font style="color:rgb(0,0,0);">meta/main.yml: </font>**<font style="color:rgb(0,0,0);">用于存放roles的元数据，如作者、角色的作用描述以及</font><font style="color:rgb(223,64,42);">角 色的依赖关系</font><font style="color:rgb(0,0,0);">. </font>

**<font style="color:rgb(0,0,0);">tasks/main.yml: </font>**<font style="color:rgb(0,0,0);">用于存放角色需要执行的主要任务，也可以在main.yml 中导入其他的task­­­­incloud，import.</font>
