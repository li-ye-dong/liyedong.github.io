# <font style="color:rgb(51, 51, 51);">主机硬件配置说明</font>
<font style="color:rgb(119, 119, 119);">提示：系统尽量别带图形界面，图形比较吃内存。</font>

| **<font style="color:rgb(51, 51, 51);">序号</font>** | **<font style="color:rgb(51, 51, 51);">主机名</font>** | **<font style="color:rgb(51, 51, 51);">IP地址</font>** | **<font style="color:rgb(51, 51, 51);">角色</font>** | **<font style="color:rgb(51, 51, 51);">系统个环境</font>** | **<font style="color:rgb(51, 51, 51);">硬件配置</font>** |
| :--- | :--- | :--- | :--- | :--- | :--- |
| <font style="color:rgb(51, 51, 51);">1</font> | <font style="color:rgb(51, 51, 51);">master</font> | <font style="color:rgb(51, 51, 51);">10.4.7.30</font> | <font style="color:rgb(51, 51, 51);">管理节点</font> | <font style="color:rgb(51, 51, 51);">CentOS 7.9</font> | <font style="color:rgb(51, 51, 51);">2CPU/4G内存/50G存储</font> |
| <font style="color:rgb(51, 51, 51);">2</font> | <font style="color:rgb(51, 51, 51);">node1</font> | <font style="color:rgb(51, 51, 51);">10.4.7.31</font> | <font style="color:rgb(51, 51, 51);">工作节点</font> | <font style="color:rgb(51, 51, 51);">CentOS 7.9</font> | <font style="color:rgb(51, 51, 51);">2CPU/4G内存/50G存储</font> |
| <font style="color:rgb(51, 51, 51);">3</font> | <font style="color:rgb(51, 51, 51);">node2</font> | <font style="color:rgb(51, 51, 51);">10.4.7.32</font> | <font style="color:rgb(51, 51, 51);">工作节点</font> | <font style="color:rgb(51, 51, 51);">CentOS 7.9</font> | <font style="color:rgb(51, 51, 51);">2CPU/4G内存/50G存储</font> |


# 每台节点配置
#### <font style="color:rgb(51, 51, 51);">主机名配置</font>
```plain
...
#### <font style="color:rgb(51, 51, 51);">主机名与IP地址解析</font>
<font style="color:rgb(51, 51, 51);">提示：以下环境配置需在所有主机都要执行</font>

<font style="color:rgb(119, 119, 119);">配置本地主机名与IP地址解析（为了后面对集群节点的调用，企业中推荐使用内部DNS服务器）</font>
...
#### <font style="color:rgb(51, 51, 51);">SSH-Key密钥认证</font>
<font style="color:rgb(119, 119, 119);">在</font>**<font style="color:rgb(65, 131, 196);">master</font>**<font style="color:rgb(119, 119, 119);">节点上生成密钥文件，拷贝到其它节点后测试免密登录。</font>

```plain
# ssh-keygen
# for i in 30 31; do ssh-copy-id 10.4.7.$i; done
# ssh node1
# ssh node2
```

#### <font style="color:rgb(51, 51, 51);">升级操作系统内核</font>
<font style="color:rgb(119, 119, 119);">内核版本低会导致一些致命的问题，所有主机均需要操作。</font>
...
#### <font style="color:rgb(51, 51, 51);">转发 IPv4 并让 iptables 看到桥接流量</font>
```plain
添加网桥的网络转发及内核转发配置文件
...
<font style="color:rgb(119, 119, 119);">为了让你的 Linux 节点上的 iptables 能够正确地查看桥接流量</font>
...
#### <font style="color:rgb(51, 51, 51);">配置ipvs功能</font>
<font style="color:rgb(119, 119, 119);">在kubernetes中Service有两种代理模型，一种是基于iptables的，一种是基于ipvs，两者对比ipvs的性能要高，如果想要使用ipvs模型，需要手动载入ipvs模块</font>
...
#### <font style="color:rgb(51, 51, 51);">关闭SWAP分区</font>
<font style="color:rgb(119, 119, 119);">swap是一种类似于Windows的虚拟内存，就是当内存不足的时候，把一部分硬盘空间虚拟成内存使用从而解决内存容量不足的情况，kubernetes强制要求禁用，否则影响集群性能。</font>
...
#### <font style="color:rgb(51, 51, 51);">Docker准备</font>
<font style="color:rgb(119, 119, 119);">所有集群主机均需操作。</font>
...
#### <font style="color:rgb(51, 51, 51);">配置Cgroup驱动程序</font>
<font style="color:rgb(119, 119, 119);">在 Linux 上，控制组（CGroup）用于限制分配给进程的资源，官方建议配置容器运行时和 kubelet 使用 systemd （systemd是Linux系统第一个初始进程）作为容器的控制组（CGroup）， 以此使系统更为稳定 。</font>
...
# <font style="color:rgb(51, 51, 51);">kubernetes 1.23.0 集群部署</font>
#### <font style="color:rgb(51, 51, 51);">集群软件版本说明</font>
| | **<font style="color:rgb(51, 51, 51);">kubeadm</font>** | **<font style="color:rgb(51, 51, 51);">kubelet</font>** | **<font style="color:rgb(51, 51, 51);">kubectl</font>** |
| --- | :--- | :--- | :--- |
| <font style="color:rgb(51, 51, 51);">版本</font> | <font style="color:rgb(51, 51, 51);">1.23.0</font> | <font style="color:rgb(51, 51, 51);">1.23.0</font> | <font style="color:rgb(51, 51, 51);">1.23.0</font> |
| <font style="color:rgb(51, 51, 51);">安装位置</font> | <font style="color:rgb(51, 51, 51);">集群所有主机</font> | <font style="color:rgb(51, 51, 51);">集群所有主机</font> | <font style="color:rgb(51, 51, 51);">集群所有主机</font> |
| <font style="color:rgb(51, 51, 51);">作用</font> | <font style="color:rgb(51, 51, 51);">初始化集群、管理集群等</font> | <font style="color:rgb(51, 51, 51);">用于接收api-server指令，对pod生命周期进行管理</font> | <font style="color:rgb(51, 51, 51);">集群应用命令行管理工具</font> |


#### <font style="color:rgb(51, 51, 51);">kubernetes YUM源准备</font>
<font style="color:rgb(119, 119, 119);">安装kubeadm、kubelet、kubectl程序（所有主机都需要安装）</font>

  
 

#### <font style="color:rgb(51, 51, 51);">谷歌YUM源</font>
```plain
[kubernetes]
name=Kubernetes
...
#### <font style="color:rgb(51, 51, 51);">阿里云YUM源</font>
<font style="color:rgb(119, 119, 119);">本实验使用阿里云YUM源</font>
...
#### <font style="color:rgb(51, 51, 51);">集群软件安装</font>
```plain
安装指定版本
# yum install -y --nogpgcheck kubeadm-1.23.0-0  kubelet-1.23.0-0 kubectl-1.23.0-0
```

#### <font style="color:rgb(51, 51, 51);">配置kubelet</font>
<font style="color:rgb(119, 119, 119);">为了实现docker使用的Cgroup驱动程序与kubelet使用的cgroup的一致性，建议修改如下文件内容。在所有节点配置。</font>
...
```shell
kubeadm init \
--apiserver-advertise-address=10.4.7.30 \
...
初始化结束后
会提示执行一些命令，创建文件，授权文件等。如下
mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config
export KUBECONFIG=/etc/kubernetes/admin.conf

然后有一句加入节点的命令，在另外两台node上执行加入集群的操作
kubeadm join 10.4.7.30:6443 --token ml3keq.ff9n63dqbo8tzyeh         
                            --discovery-token-ca-cert-hash sha256:c95d3c10f5307fa334d7c6f1efc5f3ade497cb3df8d1fbe7dd771821a9e21329

加入后使用
kubectl get nodes
结果为3个节点，显示noready，因为还没有配置网络插件
```

#### 配置网络插件
[calico.yaml](https://www.yuque.com/attachments/yuque/0/2024/yaml/40598547/1723974299983-f3dabbc8-3f6c-4c25-9f40-4ad78dca1060.yaml)
...
watch kubectl get node  #等待一会后会变成ready
```

#### <font style="color:rgb(51, 51, 51);">验证集群可用性</font>
```shell
kubectl get nodes #查看所有的节点
kubectl get cs  #查看集群健康情况
kubectl get pods -n kube-system #查看kubernetes集群pod运行情况
kubectl get pod -n calico-system #查看calico-system命名空间中pod运行情况
```

#### <font style="color:rgb(51, 51, 51);">部署Nginx程序测试</font>
```shell
部署nginx程序
...
拷贝项目到nginx站点目录
[root@master ~]# kubectl cp hello.html  nginx-696649f6f9-j8zbj:/usr/share/nginx/html 

#进入容器
#[root@master ~]# kubectl exec -it  nginx-696649f6f9-j8zbj /bin/sh

查看service状态
[root@master ~]# kubectl get services
NAME         TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE
kubernetes   ClusterIP   192.168.0.1      <none>        443/TCP        5h35m
nginx        NodePort    192.168.208.92   <none>        80:32687/TCP   18m


浏览器访问测试
curl 192.168.208.92/hello.html
curl 10.4.7.30:32687/hello.html
